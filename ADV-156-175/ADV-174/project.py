...
place_image.config(image=image)
...
# ------------------------Labels selected--------------------------
label_selected_doctor = Label(root, font=(
    "times", 12, "bold"), fg="dark olive green")
label_selected_doctor.place(relx=0.01, rely=0.3, anchor=W)

label_selected_IT = Label(root, font=(
    "times", 12, "bold"), fg="dark olive green")
label_selected_IT.place(relx=0.01, rely=0.6, anchor=W)

label_selected_chemical = Label(root, font=(
    "times", 12, "bold"), fg="dark olive green")
...
class parent():
    ...
    def doctor(doctor_name):
        ...
        label_selected_doctor['text'] = doctor_name + \
            " has been allotted to "+hospital_list[random_hospital]
    ...
    def softwareEngineer(it_professional_name):
        ...
        label_selected_IT['text'] = it_professional_name + \
            " has been allotted to "+IT_company_list[random_IT_company]

    def chemicalEngineer(chemical_engineer_name):
        company_list = ["Air Liquide", "BASF",
                        "Albemarle Corporation", "DuPont"]
        random_company = random.randint(0, 3)
        label_selected_chemical['text'] = chemical_engineer_name + \
            " has been allotted to "+company_list[random_company]
...
btn_doctor = Button(root, text="Show the hospital allotted",
                    command=obj1.getDoctor, bg="#1763A5", fg="white", relief=FLAT)

btn_doctor.place(relx=0.1, rely=0.23, anchor=CENTER)
btn_it = Button(root, text="Show the IT company allotted",
                command=obj2.getIT, bg="#1763A5", fg="white", relief=FLAT)

btn_it.place(relx=0.11, rely=0.53, anchor=CENTER)
btn_chemical = Button(root, text="Show the chemical company allotted",
                      command=obj3.getChemical, bg="#1763A5", fg="white", relief=FLAT)
